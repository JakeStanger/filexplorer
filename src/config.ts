...
function loadJavaScript(filename: string) {
...
async function loadYaml(filename: string) {
  const { parse } = await import('yaml');
  const file = await readFile(filename, 'utf-8');
  return parse(file);
}

async function loadToml(filename: string) {
  const { parse } = await import('toml');
  const file = await readFile(filename, 'utf-8');
  return parse(file);
}

async function loadCorn(filename: string) {
